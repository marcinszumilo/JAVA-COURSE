...
class BoardTestSuite {
    ...
    void testAddTaskListAverageWorkingOnTask() {
        ...
        Long sumDaysSinceCreation = project.getTaskLists().stream()
                .filter(inProgressTasks::contains)
                .flatMap(tl -> tl.getTasks().stream())
                .map(Task::getCreated)
                .map(n -> n.toEpochDay())
                .reduce( 0L, (subtotal, element) -> subtotal = subtotal - element + LocalDate.now().toEpochDay());
        int sumInstancesSinceCreation = project.getTaskLists().stream()
                .filter(inProgressTasks::contains)
                .flatMap(tl -> tl.getTasks().stream())
                .map(n -> 1)
                .reduce(0 , (sum , value) -> sum = sum + value);

        double averageDays = (double) sumDaysSinceCreation / (double) sumInstancesSinceCreation;
